...
\begin{theorem}[Naturality of Identity Functor]
  ...
  \begin{proof}
    \begin{equation*}
      \begin{aligned}
        F(f) &= F(f)\\
        F(f) \circ \id_{F(c)} &= \id_{F(c')} \circ F(f)
        ,\quad\text{(by unitality)}
      \end{aligned}
    \end{equation*}
  \end{proof}
\end{theorem}
...
\begin{theorem}[Naturality of Vertical Composition]
  ...
  \begin{proof}
    ...
    Given the naturality of $\alpha$ and $\beta$ each of the subsquares
    commutes, therefore taking either exterior path is the same implying the
    naturality of $\beta \circ \alpha$.
  \end{proof}
\end{theorem}

\begin{theorem}[Unitality of Vertical Composition]
  For categories $C,D$, functors $F,G:C\to D$ and a natural transformation $\alpha:F\to G$, the following holds:
...
\begin{definition}[Horizontal Composition]
  ...
  \begin{itemize}
    \item $c$-Components:
      \[
        \begin{aligned}
          (\beta * \alpha)_c : (G\circ F) \to (G' \circ F')
