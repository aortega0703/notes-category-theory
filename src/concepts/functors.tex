...
\begin{definition}[Functor\index{Functor}]
  ...
  and morphisms, such that it preserves~\parencite{awodey:category_theory}:
  \begin{itemize}
...
\begin{definition}[Identity Functor\index{Identity Functor}]\label{def:id_functor}
  ...
  consists of maps of~\parencite{adamek_herrlich_strecker:joy_cats}:
  \begin{itemize}
    \item Objects:
      \[(\forall c \in \C_0)
        (\id_\C(c) = c)\]
...
\begin{definition}[Functor Composition\index{Functor
  ...
  consists of maps of~\parencite{leinster:basic_category_theory}:
  \begin{itemize}
    \item Objects:
      \[(\forall a \in \A_0)
...
\begin{definition}[Faithful Functor\index{Faithful Functor}]
  ...
  injective~\parencite{leinster:basic_category_theory}:
    \[
      \begin{aligned}
        F: \C(c, c') &\to \D(F(c), F(c'))\\
        f &\mapsto F(f)
...
\begin{definition}[Full Functor\index{Full Functor}]
  ...
  \C_1$, the following function is
  surjective~\parencite{leinster:basic_category_theory}:
  \[
    \begin{aligned}
      F: \C(c, c') &\to \D(F(c), F(c'))\\
      f &\mapsto F(f)
