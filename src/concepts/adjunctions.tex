...
\begin{definition}[Transpose\index{Transpose}\label{def:transpose}]
  ...
  \[
    ...
    \end{aligned}
  \]
  Such that $\bar{\bar{u}} = u$ and $\bar{\bar{v}} = v$.
  Here $\bar{f}$ is called the transpose of $f$.
\end{definition}
  ...
  \begin{proof}
    ...
    \begin{description}
      ...
      \item[$(\impliedby)$] Consider the function:
        ...
        expressed as $\phi(\bar{v})$ for a unique $\bar{v}:L(c)\to d$,
        therefore $\phi$, which makes it a bijection and therefore an
        isomorphism. To show naturality of $\phi$ consider $g:c'\to c$ and
        $h:d\to d'$:
        ...
        \[
          \begin{aligned}
            \big(\mathcal{C}(g, R(h))\circ \phi_{c,d}\big)(u)
            &= \mathcal{D}(g, R(h))(R(u)\circ\eta_c)\\
            &= R(h)\circ R(u)\circ\eta_c \circ g\\
            &= R(h)\circ R(u) \circ (R\circ L)(g)\circ \eta_{c'}\\
            &= R(h\circ u\circ L(g))\circ \eta_{c'}\\
            &= \phi\big(h\circ u \circ L(g)\big)\\
            &= \phi\big(\mathcal{D}(L(g), h)(u)\big)\\
            &= \big(\phi\circ \mathcal{D}(L(g), h))(u)
          \end{aligned}
        \]
    \end{description}
  \end{proof}
...
\begin{theorem}[Naturality of $\epsilon$]
  ...
  \begin{proof}
    ...
    exists initial morphisms $(R(d), \epsilon_d)$ and $(R(d'),
    \epsilon_{d'})$. Construct the composite arrow $f\circ
    \epsilon_{d'}:(L\circ R)(d')\to d$. As $(R(d'), f\circ \epsilon_{d'})$
    ...
    triangle commute ($f\circ \epsilon_{d'} = \epsilon_d \circ (L\circ
    R)(d)$) completing the naturality condition of $\epsilon$.
  \end{proof}
\end{theorem}
