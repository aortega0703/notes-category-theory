...
\begin{definition}[Adjunction\index{Adjunction!via Initial Morphism}\label{def:adjunction_initial}]
  ...
  \[\begin{tikzcd}[ampersand replacement=\&]
    ...
    \arrow[""{name=1, anchor=center, inner sep=0}, "{R(\overline{v})}"', dashed, from=1-2, to=2-2]
...
\begin{theorem}[Naturality of $\eta$]
  ...
  \begin{proof}
    Take two objects $c,c'\in C_0$ with a morphism $f:c\to c'$. There must
    exists initial morphisms $(L(c), \eta_c)$ and $(L(c'), \eta_{c'})$.
    Construct the composite arrow $\eta_c\circ f:c\to (R\circ L)(c')$. As
    $(L(c'), \eta_c\circ f)$ is a candidate of initial morphism from $c$ to $R$,
    there must exist a unique arrow $L(f): L(c)\to L(c')$ that when lifted on
    $R$ makes the triangle commute ($\eta_c\circ f = (R\circ L)(f) \circ
    \eta_c$) completing the naturality condition of $\eta$.

    % https://q.uiver.app/?q=
    \[\begin{tikzcd}[ampersand replacement=\&]
      {L(c)} \& {(R\circ L)(c)} \& c \\
      {L(c')} \& {(R\circ L)(c')} \& {c'}
      \arrow["{\eta_c}"', from=1-3, to=1-2]
      \arrow["{\eta_{c'}}", from=2-3, to=2-2]
      \arrow["f", from=1-3, to=2-3]
      \arrow["{\eta_c\circ f}"{description}, from=1-3, to=2-2]
      \arrow["{(R\circ L)(f)}"', dashed, from=1-2, to=2-2]
      \arrow["{L(f)}"', dashed, from=1-1, to=2-1]
      \arrow[color={rgb,255:red,92;green,92;blue,214}, dotted, from=1-1, to=1-2]
      \arrow[color={rgb,255:red,92;green,92;blue,214}, dotted, from=2-1, to=2-2]
    \end{tikzcd}\]
  \end{proof}
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    \begin{description}
      \item[$(\implies)$] \todo{Complete Proof}
    \end{description}
  \end{proof}
\end{theorem}
...
\begin{definition}[Adjunction\index{Adjunction!via Terminal Morphism}\label{def:adjunction_terminal}]
  ...
  \varepsilon_d: (L\circ R)(d) \to d)$ from every $d\in D$ to $L$:
  \parencite{awodey:category_theory}
  ...
  \[\begin{tikzcd}[ampersand replacement=\&]
    ...
    \arrow[""{name=1, anchor=center, inner sep=0}, "{L(\overline{u})}"', dashed, from=1-2, to=2-2]
...
\begin{theorem}[Naturality of $\varepsilon$]
  In Definition \ref{def:adjunction_terminal}, $\varepsilon: L\circ R \to \id_d$
  is a natural transformation.

  \begin{proof}
    Take two objects $d,d'\in D_0$ with a morphism $f:d'\to d$. There must
    exists initial morphisms $(R(d), \varepsilon_d)$ and $(R(d'),
    \varepsilon_{d'})$. Construct the composite arrow $f\circ
    \varepsilon_{d'}:(L\circ R)(d')\to d$. As $(R(d'), f\circ \varepsilon_{d'})$
    is a candidate of terminal morphism from $L$ to $d$, there must exist a
    unique arrow $R(f): R(d')\to R(c)$ that when lifted on $L$ makes the
    triangle commute ($f\circ \varepsilon_{d'} = \varepsilon_d \circ (L\circ
    R)(d)$) completing the naturality condition of $\varepsilon$.
  \end{proof}
\end{theorem}

\begin{theorem}[Adjunction Definitions]
  Definitions \ref{def:adjunction_initial} and \ref{def:adjunction_terminal}
  are equivalent.
